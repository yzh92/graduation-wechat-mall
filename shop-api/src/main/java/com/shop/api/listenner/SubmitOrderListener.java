...
import com.shop.bean.app.dto.ShopCartItemDiscountDto;
...
import com.shop.bean.enums.OrderStatus;
import com.shop.bean.event.SubmitOrderEvent;
import com.shop.bean.model.*;
import com.shop.bean.order.SubmitOrderOrder;
...
public class SubmitOrderListener {
    ...
    public void defaultSubmitOrderListener(SubmitOrderEvent event) {
        ...
        if (userAddrOrder == null) {
            throw new ShopBindException("请填写收货地址");
        }
        ...
        for (ShopCartOrderDto shopCartOrderDto : shopCartOrders) {
            ...
            // 订单信息
            com.shop.bean.model.Order order = new com.shop.bean.model.Order();
        ...
        skuStocksMap.forEach((key, sku) -> {
            ...
            if (skuMapper.updateStocks(sku) == 0) {
                ...
                throw new ShopBindException("商品：[" + sku.getProdName() + "]库存不足");
            }
        });
        ...
        prodStocksMap.forEach((prodId, prod) -> {
            ...
            if (productMapper.updateStocks(prod) == 0) {
                ...
                throw new ShopBindException("商品：[" + prod.getProdName() + "]库存不足");
            }
        });
    ...
    private Product checkAndGetProd(Long prodId, ShopCartItemDto shopCartItem, Map<Long, Product> prodStocksMap) {
        ...
        if (product.getStatus() != 1) {
            throw new ShopBindException("商品[" + product.getProdName() + "]已下架");
        }
        ...
        if (product.getTotalStocks() != -1 && mapProduct.getTotalStocks() > product.getTotalStocks()) {
            throw new ShopBindException("商品：[" + product.getProdName() + "]库存不足");
        }
